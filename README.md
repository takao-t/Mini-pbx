...
通話音量が小さいと感じられる場合にはR2,R4を除去(オープン)、R1,R3をショート(0Ω)してください。R1,R2,R3,R4はVout出力のDCバイアスをmid-railにするためと-3dbのアッテネータとして使われています。このPBXでは音声接続にアナログスイッチを使っているのでDCバイアスの必要はない(そもそも0.1uFが入っている)ので除去とショートしても問題はありません。
